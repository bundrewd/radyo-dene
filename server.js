...
const app = express();
// IP adreslerini doğru okuyabilmek için trust proxy ayarını açıyoruz (Sunucuya taşıdığımızda gerekecek)
...
let activeListeners = new Map(); // Dinleyicileri tutacağımız radar
...
app.get('/api/current-track', async (req, res) => {
    // Dinleyici Takibi (Son 12 saniye içinde sinyal gönderenleri aktif sayarız)
    const userIp = req.ip || req.connection.remoteAddress;
    activeListeners.set(userIp, Date.now());
    
    const now = Date.now();
    activeListeners.forEach((lastSeen, ip) => {
        if (now - lastSeen > 12000) activeListeners.delete(ip); // Düşenleri sil
    });
    ...
    try {
        ...
        if (response.data && response.data.item) {
            res.json({
                ...
                album_art: response.data.item.album.images[0].url, // Albüm Kapağı
                listeners: listenerCount // Dinleyici Sayısı
            });
        } else {
